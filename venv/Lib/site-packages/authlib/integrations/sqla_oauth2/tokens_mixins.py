...
class OAuth2TokenMixin(TokenMixin):
    ...
    def default_refresh_token_revoked_at(self):
        return int(time.time()) + 864000  # 900 seconds = 15 minutes
    ...
    refresh_token_revoked_at = Column(Integer, nullable=False, default=default_refresh_token_revoked_at)
    ...
    def is_revoked(self):
        # Check if access_token_revoked_at already expired
        if self.access_token_revoked_at < time.time() or self.refresh_token_revoked_at < time.time():
            print("TRUE")
            return True
        return False
